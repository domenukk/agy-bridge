...
impl ToolRegistry {
    ...
    /// Try to build a definition for a named tool, logging on failure.
    fn try_definition<'a>(
        name: &'a str,
        tool: &'a dyn ErasedTool,
    ) -> Option<(&'a str, ToolDefinition)> {
        match tool.definition() {
            Ok(def) => Some((name, def)),
            Err(e) => {
                tracing::warn!(tool = %name, error = %e, "Skipping tool with invalid schema");
                None
            }
        }
    }
    ...
    pub fn definitions(&self) -> Vec<ToolDefinition> {
        self.tools
            ...
            .filter_map(|(name, tool)| {
                Self::try_definition(name, tool.as_ref()).map(|(_, def)| def)
            })
            .collect()
    }
    ...
    pub async fn dispatch(
        ...
        tool.call_erased(args, ctx).await
    }
    ...
    pub fn iter(&self) -> impl Iterator<Item = (&String, ToolDefinition)> + '_ {
        self.tools.iter().filter_map(|(name, tool)| {
            Self::try_definition(name, tool.as_ref()).map(|(_, def)| (name, def))
        })
    }
}
...
impl<'a> IntoIterator for &'a ToolRegistry {
    type Item = (&'a String, ToolDefinition);
    type IntoIter = Box<dyn Iterator<Item = (&'a String, ToolDefinition)> + 'a>;

    fn into_iter(self) -> Self::IntoIter {
        Box::new(self.tools.iter().filter_map(|(name, tool)| {
            ToolRegistry::try_definition(name, tool.as_ref()).map(|(_, def)| (name, def))
        }))
    }
}
...
mod tests {
    ...
    async fn registry_dispatch_invalid_args() {
        ...
        assert!(
            result.is_err(),
            "Expected deserialization error for wrong type"
        );
    ...
    fn into_iter_yields_all_tool_name_definition_pairs() {
        ...
        let mut pairs: Vec<(String, String)> = (&d)
            .into_iter()
            .map(|(name, def)| (name.clone(), def.name))
            .collect();
        pairs.sort();
