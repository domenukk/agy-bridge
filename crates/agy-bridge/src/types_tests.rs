...
fn test_step_full_roundtrip() {
    let step = Step {
        ...
        parent_trajectory_id: "traj_parent".to_string(),
        depth: 1,
        error: String::new(),
        http_code: 0,
    ...
    assert_eq!(parsed.parent_trajectory_id, "traj_parent");
...
#[test]
fn test_stop_reason_roundtrip_and_parse() {
    for (variant, wire) in [
    ...
    assert_eq!(
        "MAX_MODEL_CALLS_EXCEEDED".parse::<StopReason>().unwrap(),
        StopReason::MaxModelCallsExceeded
    );
    assert_eq!(
        "QUOTA_EXHAUSTED".parse::<StopReason>().unwrap(),
        StopReason::QuotaExhausted
    );
}

#[test]
fn test_modality_token_count_roundtrip() {
    for (m, wire) in [
        (Modality::Text, "\"TEXT\""),
    ...
    let detail = ModalityTokenCount {
        modality: Modality::Image,
        token_count: 256,
    };
    let json = serde_json::to_string(&detail).unwrap();
    let parsed: ModalityTokenCount = serde_json::from_str(&json).unwrap();
